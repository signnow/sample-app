...
namespace Samples\EmbeddedSenderWithFormAndFirstSigner;
...
class SampleController implements SampleControllerInterface
{
    ...
    {
        return new Response(
            view('EmbeddedSenderWithFormAndFirstSigner::index')->render(),
            200,
            [
    ...
    public function handlePost(Request $request): Response
    {
        ...
        switch ($action) {
            ...
            case 'create_signing_url':
                return $this->createSigningUrl($request, $apiClient);
            case 'invite-status':
    ...
    private function createSigningUrl(Request $request, ApiClient $apiClient): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'signing_url' => $signingLinkResponse['signing_url']
        ]);
    }
    ...
    private function updateDocumentGroupRecipients(
        ...
        // Create updated recipients with different email addresses based on role
        $updatedRecipients = [];
    ...
    {
        // Return URL after embedded sending to redirect to signing page
        $redirectUrl = config('app.url')
            . '/samples/EmbeddedSenderWithFormAndFirstSigner?' .
            http_build_query([
                'page' => 'signing-page',
    ...
    private function createSigningLink(ApiClient $apiClient, string $documentGroupId): array
    {
        // Generate limited token for Document Group
        $limitedTokenResponse = $this->generateLimitedToken($apiClient, $documentGroupId);
        if (!$limitedTokenResponse['success']) {
            return $limitedTokenResponse;
        }

        $accessToken = $limitedTokenResponse['access_token'];

        // Build signing URL with parameters - redirect to status page after signing
        $redirectUrl = config('app.url')
            . '/samples/EmbeddedSenderWithFormAndFirstSigner?' .
            http_build_query([
                'page' => 'status-page',
    ...
    private function generateLimitedToken(ApiClient $apiClient, string $documentGroupId): array
    {
