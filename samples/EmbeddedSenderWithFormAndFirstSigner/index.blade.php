...
<head>
    ...
    <title>ISV With Form And One Click Send - SignNow Sample</title>
...
<body>
    ...
    <script>
        ...
        async function initSigningPage() {
            const signingContent = document.getElementById('signing-content');
            const documentGroupId = getQueryParam('document_group_id');

            if (!documentGroupId) {
                signingContent.innerHTML = `
                    <div class="alert alert-danger">
                        Error: Document group ID not found
                    </div>
                `;
                return;
            }
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        data.forEach(doc => {
                            ...
                            li.querySelector('.download-document').addEventListener('click', async () => {
                                try {
                                    const res = await fetch('/api/samples/EmbeddedSenderWithFormAndFirstSigner', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            document_group_id: documentGroupId,
