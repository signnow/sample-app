...
<head>
    ...
    <title>Embedded Sender With Form DG - Adjunct Professor Contract - SignNow Sample</title>
...
<body>
    ...
    <div id="form-page" class="container-block">
        <h4>Enter Adjunct Professor Contract Information</h4>
        <form id="signer-form">
    ...
    <div id="status-page" class="container-block" style="display: none;">
        <h4>Adjunct Professor Contract Status & Final Document</h4>
        <p>You can check signing status and download the completed adjunct professor contract here.</p>
    ...
    <script>
        ...
        async function initFormPage() {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('/api/samples/EmbeddedSenderWithFormDGAdjunct', {
                        method: 'POST',
                    ...
                    if (result.success) {
                        ...
                        alert('Error: ' + (result.message || 'Failed to prepare document group'));
                    }
                } catch (error) {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        data.forEach(doc => {
                            ...
                            li.querySelector('.download-document').addEventListener('click', async () => {
                                try {
                                    const res = await fetch('/api/samples/EmbeddedSenderWithFormDGAdjunct', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            document_group_id: documentGroupId,
                                    ...
                                    const filename = res.headers.get('Content-Disposition')?.match(/filename="([^"]+)"/)?.[1] || 'ADJUNCT PROFESSOR CONTRACT.pdf';
