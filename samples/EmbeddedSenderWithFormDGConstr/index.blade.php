...
<head>
    ...
    <title>Embedded Sender With Form DG - Construction Project Contract - SignNow Sample</title>
...
<body>
    ...
    <div id="form-page" class="container-block">
        <h4>Enter Construction Project Contract Information</h4>
        <form id="signer-form">
    ...
    <div id="status-page" class="container-block" style="display: none;">
        <h4>Construction Project Contract Status & Final Document</h4>
        <p>You can check signing status and download the completed construction project contract here.</p>
    ...
    <script>
        ...
        async function initFormPage() {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('/api/samples/EmbeddedSenderWithFormDGConstr', {
                        method: 'POST',
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        data.forEach(doc => {
                            ...
                            li.querySelector('.download-document').addEventListener('click', async () => {
                                try {
                                    const res = await fetch('/api/samples/EmbeddedSenderWithFormDGConstr', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            document_group_id: documentGroupId,
