...
<head>
    ...
    <title>ISV with Form and One Click Send - Basic Pre-fill - SignNow Sample</title>
    ...
    <script>
        ...
        async function initFormPage() {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('/api/samples/ISVWithFormAndOneClickSendBasicPrefill', {
                        method: 'POST',
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        data.forEach(doc => {
                            ...
                            li.querySelector('.download-document').addEventListener('click', async () => {
                                try {
                                    const res = await fetch('/api/samples/ISVWithFormAndOneClickSendBasicPrefill', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            document_group_id: documentGroupId,
