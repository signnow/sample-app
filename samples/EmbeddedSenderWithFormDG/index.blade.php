...
<head>
    ...
    <title>Embedded Sender With Form DG - SignNow Sample</title>
    ...
    <script>
        ...
        async function initFormPage() {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('/api/samples/EmbeddedSenderWithFormDG', {
                        method: 'POST',
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        data.forEach(doc => {
                            ...
                            li.querySelector('.download-document').addEventListener('click', async () => {
                                try {
                                    const res = await fetch('/api/samples/EmbeddedSenderWithFormDG', {
                                        method: 'POST',
                                        headers: { 'Content-Type': 'application/json' },
                                        body: JSON.stringify({
                                            document_group_id: documentGroupId,
